			...
			if yes_or_no > 50 or yes_or_no < -50 then
				...
				for _, player in ipairs(minetest.get_connected_players()) do
					...
					if nodepos.y > 0 then
						...
						if time_of_day < 17000 and time_of_day > 6000 then
								...
								if pos.y < 32 then
									if node.name == "default:dirt_with_grass" or node.name == "default:dirt_with_dry_grass" then
										minetest.add_particlespawner({
											amount = 15,
											time = 0.5,
											minpos = {x = pos.x-10, y = pos.y+25, z = pos.z-10},
											maxpos = {x = pos.x+10, y = pos.y+30, z = pos.z+10},
											minvel = {x = wind/3, y = math.random(-3,-5), z = wind/3},
											maxvel = {x = wind/3, y = math.random(-3,-5), z = wind/3},
